...
pub fn slice_iter_filter(
    values: &dyn Array,
    predicate: &BooleanArray,
) -> Result<ArrayRef, ArrowError> {
    let predicate = FilterBuilder::new_with_slice_iter(predicate).build();
    filter_array(values, &predicate)
}

pub fn index_iter_filter(
    values: &dyn Array,
    predicate: &BooleanArray,
) -> Result<ArrayRef, ArrowError> {
    let predicate = FilterBuilder::new_with_index_iter(predicate).build();
    filter_array(values, &predicate)
}
...
impl FilterBuilder {
    ...
    pub fn new_with_slice_iter(filter: &BooleanArray) -> Self {
        let filter = match filter.null_count() {
            0 => filter.clone(),
            _ => prep_null_mask_filter(filter),
        };
        ...
        let count = filter_count(&filter);
        let strategy = IterationStrategy::SlicesIterator;
    ...
    pub fn new_with_index_iter(filter: &BooleanArray) -> Self {
        let filter = match filter.null_count() {
            0 => filter.clone(),
            _ => prep_null_mask_filter(filter),
        };
        ...
        let count = filter_count(&filter);
        let strategy = IterationStrategy::IndexIterator;
